...
static __weak IPFTestRunner *s_currentTestRunner;
...
- (void)startTest:(IPFTestRunnerCallback)callback
{
  ...
  if (configuration.protocol == IPFTestRunnerProtocolUDP) {
    set_protocol(test, Pudp); // Pudp is defined in iperf_api.h
    
    // iPerf3 UDP defaults to 1 Mbps. We must explicitly set a target rate.
    // Setting it to 0 usually means "unlimited" in modern iPerf3 builds.
    uint64_t rate = configuration.targetBitrate > 0 ? configuration.targetBitrate : 100000;
    iperf_set_test_blksize(test, 1140);
    iperf_set_test_rate(test, rate);
  } else {
    set_protocol(test, Ptcp); // Explicitly set TCP
  }
  ...
  dispatch_async(dispatch_get_global_queue(QOS_CLASS_USER_INTERACTIVE, 0), ^{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
      // 1. FREE THE C-STRUCT NO MATTER WHAT
      iperf_free_test(test);
      
      // 2. CHECK IF SWIFT HAS ALREADY DESTROYED THIS OBJECT
      typeof(self) strongSelf = weakSelf;
      if (!strongSelf) {
        return; // The object is gone (user hit stop), exit safely!
      }
      
      // 3. IF IT STILL EXISTS, CLEAN UP SAFELY
      IPFTestRunnerStatus callbackStatus = status;
      IPFTestRunnerCallback callback = strongSelf->_callback;
      
      s_currentTestRunner = nil;
      strongSelf->_callback = nil;
      ...
      if (i_errno != IENONE) {
                NSLog(@"[iPerf3] UDP Test Failed. Server said: %s", iperf_strerror(i_errno));
            }
      if (callback) {
  ...
  - (void)handleStatsCallback:(struct iperf_test *)test
  {
    ...
    // See iperf_reporter_callback
    {
      ...
      if (stream) {
        ...
        status.packetLossPercent = lost_percent;
        status.jitter = avg_jitter * 1000.0;
